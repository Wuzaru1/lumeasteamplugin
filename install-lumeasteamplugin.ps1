...
$SteamRegistry = "HKLM:\SOFTWARE\WOW6432Node\Valve\Steam"
$SteamPath = (Get-ItemProperty -Path $SteamRegistry -ErrorAction Stop).InstallPath
...
$PluginsDir = Join-Path $SteamPath "steamui\plugins"
...
# ---------------------------
$MillenniumDir = Join-Path $PluginsDir "Millennium"
...
$VersionContent = @{ version = $Asset.name } | ConvertTo-Json
